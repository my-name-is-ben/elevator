from modules.elevator import ElevatorSimV3
import unittest
...
class TestElevatorModule(unittest.TestCase):
  ...
  def test_starting_floor_only_accepts_integers(self):
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3('not an int', [1, 2, 3])
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3(['not', 'an', 'int'], [1, 2, 3])
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3([], [1, 2, 3])
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3(None, [1, 2, 3])

  def test_starting_floor_only_accepts_positive_integers(self):
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(0, [1, 2, 3])
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(-1, [1, 2, 3])
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(-74674, [1, 2, 3])

  def test_floor_list_only_accepts_lists(self):
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3(4, 'a')
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3(4, None)
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3(4, 1)
    with self.assertRaises(TypeError):
      sim = ElevatorSimV3(4, 1, 2, 3)

  def test_floor_list_only_accepts_positive_integers(self):
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(4, [0])
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(4, [1, 2, 3, -1])
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(4, [1, 2, 3, 0])
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(4, [None])
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(4, [1, 2, 3, None])
    with self.assertRaises(ValueError):
      sim = ElevatorSimV3(4, [1, 2, 3, 'test'])

  def test_travel_time_calculation_is_correct(self):
    sim1 = ElevatorSimV3(4, [])
    self.assertEqual(sim1.get_total_travel_time(), 0)
    sim2 = ElevatorSimV3(1, [2])
    self.assertEqual(sim2.get_total_travel_time(), 10)
    sim2 = ElevatorSimV3(1, [2, 3, 2])
    self.assertEqual(sim2.get_total_travel_time(), 30)
    sim2 = ElevatorSimV3(2, [2, 2, 2])
    self.assertEqual(sim2.get_total_travel_time(), 0)
    sim2 = ElevatorSimV3(1, [100])
